...
# Generator script — run once to produce 2_Financial.py
import textwrap, os
...
src = textwrap.dedent('''
    """Digital Twin AI - Finance (full-featured)"""
    import os, sys
    ...
    st.set_page_config(page_title="Finance - Digital Twin AI", page_icon="\\U0001f4b0",
    ...
    metric_row([
        dict(icon="\\U0001f49a", label="Total Income",
             value=f"${summary.get(\'total_income\',0):,.0f}",
             sub="All time", trend_up=True,
             accent="linear-gradient(90deg,#10B981,#059669)"),
        dict(icon="\\U0001f534", label="Total Expenses",
             value=f"${summary.get(\'total_expenses\',0):,.0f}",
             sub="All time", trend_up=False,
             accent="linear-gradient(90deg,#EF4444,#DC2626)"),
        dict(icon="\\U0001f4b0", label="Net Savings",
             value=f"${summary.get(\'net_savings\',0):,.0f}",
             sub="Income minus Expenses",
             trend_up=summary.get("net_savings", 0) >= 0,
             accent="linear-gradient(90deg,#06B6D4,#0284C7)"),
        dict(icon="\\U0001f4c8", label="Monthly Trend",
             value=f"${monthly_trend:,.0f}",
             sub="Current month net", trend_up=monthly_trend >= 0,
             accent="linear-gradient(90deg,#8B5CF6,#7C3AED)"),
        dict(icon="\\U0001f4cb", label="Total Records",
    ...
    if records:
        ...
        with c1:
            section_header("\\U0001f4c8", "Income vs Expenses Over Time")
            inc_s = sorted([r for r in records if r["record_type"] == "income"],
                           key=lambda x: x["date"])
            exp_s = sorted([r for r in records if r["record_type"] == "expense"],
                           key=lambda x: x["date"])
            fig = go.Figure()
            if inc_s:
                fig.add_trace(go.Scatter(
                    x=[r["date"][:10] for r in inc_s],
                    y=[r["amount"] for r in inc_s],
            ...
            if exp_s:
                fig.add_trace(go.Scatter(
                    x=[r["date"][:10] for r in exp_s],
                    y=[r["amount"] for r in exp_s],
        ...
        with c2:
            section_header("\\U0001f355", "Spending by Category")
            ...
            if exp_recs:
                ...
                empty_state("\\U0001f355", "No expense records yet")
        ...
        with c3:
            section_header("\\U0001f4ca", "Monthly Income vs Expenses")
            ...
            if months:
                ...
                fig3.add_trace(go.Bar(
                    x=months, y=[m_inc.get(m, 0) for m in months],
                    name="Income", marker_color="#10B981", opacity=0.85,
                ))
                fig3.add_trace(go.Bar(
                    x=months, y=[m_exp.get(m, 0) for m in months],
        ...
        with c4:
            section_header("\\U0001f49a", "Income by Category")
            ...
            if inc_recs:
                ...
                empty_state("\\U0001f49a", "No income records yet")

        st.markdown("<div style=\'height:1rem\'></div>", unsafe_allow_html=True)

        # ── CHART ROW 3 ───────────────────────────────────────────────────────
        section_header("\\U0001f504", "Recurring Frequency Breakdown")
    ...
    # ── FILTER BAR ────────────────────────────────────────────────────────────
    section_header("\\U0001f50d", "Filter and Manage Records")
    ...
    if filtered:
        rows_ex = [{
            "ID": r["id"], "Type": r["record_type"], "Amount": r["amount"],
            ...
            "Goal Impact": r.get("goal_impact") or "",
        } for r in filtered]
    ...
    with col_form:
        ...
        edit_rec = (next((r for r in records if r["id"] == st.session_state.edit_rec_id), None)
        ...
        with st.form(f"fin_form_{st.session_state.edit_rec_id or \'new\'}"):
            ...
            try:
                def_date = (datetime.fromisoformat(edit_rec["date"][:10]).date()
                            if edit_rec else datetime.today().date())
            except Exception:
        ...
        if submitted:
            ...
            if errs:
                ...
                try:
                    if editing and edit_rec:
                        client.put(
                            f"/api/financial/records/{edit_rec[\'id\']}",
    ...
    with col_table:
        ...
        if filtered:
            ...
            empty_state("\\U0001f4b0", "No matching records", "Try adjusting your filters.")
''').lstrip('\n')
...
try:
    ast.parse(src)
    print(f"WRITTEN OK  |  {src.count(chr(10))} lines  |  {len(src)} chars")
